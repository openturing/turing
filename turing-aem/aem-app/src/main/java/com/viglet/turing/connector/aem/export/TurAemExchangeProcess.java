 ...
 */
 ...
 public class TurAemExchangeProcess {
   ...
   private Collection<TurAemAttribExchange> attributeExchange(
     ...
     attributeSpecifications.forEach(
         attributeSpecification ->
             attribExchanges.add(
                 TurAemAttribExchange.builder()
       ...
       try {
         mapper
             ...
             .writeValue(
                 exportFile,
                 new TurAemExchange(
                     turAemSources.stream()
                         .map(
                             turAemSource ->
                                 TurAemSourceExchange.builder()
                                     .id(turAemSource.getId())
                                     .url(turAemSource.getUrl())
                                     .attributes(
                                         attributeExchange(
                                             turAemSource.getAttributeSpecifications()))
         ...
         File zipFile =
             new File(tmpDir.getAbsolutePath().concat(File.separator + folderName + ".zip"));
   ...
   public void importFromMultipartFile(MultipartFile multipartFile) {
     ...
     File exportFile =
         new File(extractFolder.getAbsolutePath().concat(File.separator + EXPORT_FILE));
   ...
   private void setSources(TurAemExchange turAemExchange) {
     turAemExchange.getSources().stream()
         .filter(
             turAemSourceExchange ->
   ...
   private @NotNull TurAemModel setModel(TurAemModelExchange model) {
     return turAemModelRepository.save(
         TurAemModel.builder().type(model.getType()).className(model.getClassName()).build());
   }
   ...
   private @NotNull TurAemTargetAttribute setTargetAttribute(
       TurAemModel turAemModel, TurAemTargetAttrExchange targetAttr) {
     return turAemTargetAttributeRepository.save(
         TurAemTargetAttribute.builder()
             .name(targetAttr.getName())
             .turAemModel(turAemModel)
             .build());
   }
 
   private void setSourceAttributes(
       TurAemTargetAttrExchange targetAttr, TurAemTargetAttribute turAemTargetAttribute) {
     targetAttr
         .getSourceAttrs()
         .forEach(
             sourceAttr ->
                 turAemSourceAttributeRepository.save(
                     TurAemSourceAttribute.builder()
   ...
   private void setFacetNames(TurAemSourceExchange turAemSourceExchange, TurAemSource turAemSource) {
     turAemSourceExchange
         .getAttributes()
         .forEach(
             attribute -> setFacetName(attribute, setAttributeMapping(turAemSource, attribute)));
   }
 
   private void setFacetName(
       TurAemAttribExchange attribute, TurAemAttributeSpecification turAemAttributeSpecification) {
     turAemAttributeSpecification.setFacetNames(attribute.getFacetName());
   }
 
   private TurAemAttributeSpecification setAttributeMapping(
       TurAemSource turAemSource, TurAemAttribExchange attribute) {
     return turAemAttributeSpecificationRepository.save(
         TurAemAttributeSpecification.builder()
