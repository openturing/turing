...
public class TurSpotlightExtraFields implements ExtAttributeInterface {
    private static final ContextLogger log = ContextLogger.getLogger(TurSpotlightExtraFields.class.getName());

    @Override
    public TurMultiValue consume(TuringTag tag, ContentInstance ci, AttributeData attributeData,
                                 IHandlerConfiguration config) throws Exception {
        log.debug("Executing TurSpotlightExtraFields");
        List<TurSpotlightContent> turSpotlightContents = new ArrayList<TurSpotlightContent>();
       for(AttributedObject attributedObject : Arrays.asList(ci.getRelations(tag.getSrcAttributeRelation().get(0)))){
            try {
                ...
                TurSpotlightContent turSpotlightContent = new TurSpotlightContent((Integer)position, (String) title,
