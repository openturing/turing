...
public class TurSpotlightExtraFields implements ExtAttributeInterface {
  private static final ContextLogger log =
      ContextLogger.getLogger(MethodHandles.lookup().lookupClass());
  ...
  public TurMultiValue consume(
    ...
    try {
      Arrays.asList(ci.getRelations(tag.getSrcAttributeRelation().get(0)))
          .forEach(
              attributedObject -> {
                try {
                  Object position =
                      attributedObject.getAttributeValue("POSITION-TUR-SPOTLIGHT-CONTENT");
                  Object title = attributedObject.getAttributeValue("TITLE-TUR-SPOTLIGHT-CONTENT");
                  Object content =
                      attributedObject.getAttributeValue("CONTENT-TUR-SPOTLIGHT-CONTENT");
                  Object link = attributedObject.getAttributeValue("LINK-TUR-SPOTLIGHT-CONTENT");
                  String contentText = "";
                  if (((String) content).length() == 40) {
                    ContentInstance contentInstance =
                        (ContentInstance)
                            ManagedObject.findByContentManagementId(
                                new ManagedObjectVCMRef(content.toString()));
                  ...
                  TurSpotlightContent turSpotlightContent =
                      new TurSpotlightContent(
                          (int) position, (String) title, contentText, (String) link);
