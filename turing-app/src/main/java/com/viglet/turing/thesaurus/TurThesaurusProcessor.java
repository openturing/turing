...
public class TurThesaurusProcessor {
  ...
  public Map<String, Object> detectTerms(Map<String, Object> attributes) {
    ...
    if (attributes != null) {
      for (Object attrValue : attributes.values()) {
        Map<String, List<String>> termsDetected =
            this.detectTerms(TurSolrField.convertFieldToString(attrValue));
  ...
  public TurEntityResults detectTerms(String text) {
    ...
    for (String word : words) {
      ...
      log.debug("word: {}", word);
      List<TurTermVariation> results =
          Arrays.stream(stringTerms).filter(t -> t.getNameLower().contains(wordLowerCase)).toList();
    ...
    for (Object wordObject : turNLPSentence.getWords().values().toArray()) {
      ...
      Map<String, TurTermVariation> variations =
          lhWords.get(TurCommonsUtils.stripAccents(turNLPWord.getWord()).toLowerCase());

      if (prevVariations != null) {
        log.debug("variations.size(): {}", variations.size());
        for (Object variationObject : variations.values().toArray()) {
          setVariations(
              entityResults, matches, turNLPWordPrev, prevVariations, turNLPWord, variationObject);
        }
      } else {
  ...
  private void setVariations(
      TurEntityResults entityResults,
      Map<TurNLPListKey<Integer>, List<String>> matches,
      TurNLPWord turNLPWordPrev,
      Map<String, TurTermVariation> prevVariations,
      TurNLPWord turNLPWord,
  ...
  private void manyWords(
    ...
    if (prevVariations.containsKey(variation.getId())) {
      log.debug(
          "Found {} {}: {} ", turNLPWordPrev.getWord(), turNLPWord.getWord(), variation.getId());
  ...
  private void singleWord(
      TurEntityResults entityResults, TurNLPWord turNLPWord, TurTermVariation variation) {
    ...
    if (this.validateTerm(turNLPWord.getWord(), variation)) {
      ...
      if (turNLPEntity != null) {
        ...
        if (!entityResults.containsKey(turNLPEntity.getCollectionName())) {
          log.debug("First Item into collection for results: {}", turNLPEntity.getCollectionName());
          List<String> lstTerm = new ArrayList<>();
          lstTerm.add(variation.getName());
          entityResults.put(turNLPEntity.getCollectionName(), lstTerm);
        } else {
          log.debug("Contains the collection for results: {}", turNLPEntity.getCollectionName());
          entityResults.get(turNLPEntity.getCollectionName()).add(variation.getName());
        }
  ...
  private void setMatches(
    ...
    for (Entry<TurNLPListKey<Integer>, List<String>> pair : matches.entrySet()) {
      ...
      this.mergeEntityResultsFromTo(
          this.checkTermIdBetweenPositions(turNLPSentence, positions, ids, matches), entityResults);
    }
  }
  ...
  public TurEntityResults checkTermIdBetweenPositions(
      TurNLPSentence turNLPSentence,
  ...
  private void briefMatches(
    ...
    if (matches.containsKey(positionsPrev)) {
      ...
      for (String id : ids) {
        processIds(
            turNLPSentence, positions, matches, entityResults, positionsPrev, filteredIds, id);
      }
    } else {
  ...
  private void processIds(
      ...
      Map<TurNLPListKey<Integer>, List<String>> matches,
      TurEntityResults entityResults,
      TurNLPListKey<Integer> positionsPrev,
  ...
  private void processValidatedTerm(
      TurEntityResults entityResults, String id, StringBuilder wordMatched) {
    log.debug("Match Found");
    String entity = this.getEntity(terms.get(id)).getName().toLowerCase();
    this.mergeEntityResultsFromTo(this.getParentTerm(terms.get(id).getTurTerm()), entityResults);
      ...
      if (relationFrom.getRelationType() == TurNLPRelationType.BT.id()) {
        ...
        for (TurTermRelationTo relationTo : relationFrom.getTurTermRelationTos()) {
          ...
          if (entityResults.containsKey(parentCollection)) {
            ...
            log.debug(
                "Parent Term Collection size: {}", entityResults.get(parentCollection).size());
          }
  ...
  private boolean validateUCSRuleCase(
      String word,
      TurTermVariation variation,
      String wordNoAccent,
      String termName,
  ...
  private boolean validateCIRuleCase(
      TurTermVariation variation, String wordLowerCaseWithAccent, String termName) {
  ...
  private boolean validateCSRuleCase(
      String word,
      TurTermVariation variation,
      String wordNoAccent,
      String termName,
