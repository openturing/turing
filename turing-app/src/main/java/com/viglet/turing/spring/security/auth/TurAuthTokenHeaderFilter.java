...
public class TurAuthTokenHeaderFilter extends OncePerRequestFilter {
  ...
  @Inject
  public TurAuthTokenHeaderFilter(
      TurUserRepository turUserRepository, TurDevTokenRepository turDevTokenRepository) {
  ...
  protected void doFilterInternal(
    ...
    if (appId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
      turDevTokenRepository
          .findByToken(appId)
          .ifPresent(
              token -> {
                TurUser turUser = turUserRepository.findByUsername(token.getCreatedBy());
                UsernamePasswordAuthenticationToken authenticationToken =
                    new UsernamePasswordAuthenticationToken(turUser, null, Collections.emptyList());
