...
public class TurSNTargetingRules {
  ...
  public String ruleExpression(TurSNTargetingRuleMethod method, List<String> trs) {
    if (method.equals(TurSNTargetingRuleMethod.AND)) return this.andMethod(trs);
    else if (method.equals(TurSNTargetingRuleMethod.OR)) return this.orMethod(trs);
    else return EMPTY;
  }
  ...
  public String andMethod(List<String> trs) {
    ...
    for (String tr : trs) {
      if (tr.contains(TWO_POINTS)) {
        ...
        if (!trMap.containsKey(attribute)) trMap.put(attribute, new ArrayList<>());
    ...
    for (Entry<String, List<String>> trEntry : trMap.entrySet()) {
      ...
      for (String trEntryValue : trEntry.getValue()) {
        targetingRuleQuery.append(" ");
        targetingRuleQuery.append(
            "%s %s:%s".formatted(targetingRuleOR, trEntry.getKey(), trEntryValue).trim());
        targetingRuleOR = "OR";
      }
  ...
  public String orMethod(List<String> trs) {
    ...
    trs.stream()
        .filter(tr -> tr.contains(TWO_POINTS))
        .forEach(
            tr ->
    ...
    return String.format(
        "%s OR (*:* %s)", String.join(" OR ", trList), String.join(" AND ", attributeList));
  }
}
