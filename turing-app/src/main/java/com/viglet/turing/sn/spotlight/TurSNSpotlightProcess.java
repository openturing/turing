...
public class TurSNSpotlightProcess {
  ...
  private void ifExistsDeleteSpotlightDependencies(TurSNSiteSpotlight turSNSiteSpotlight) {
    if (turSNSiteSpotlight != null) {
      Set<TurSNSiteSpotlightTerm> turSNSiteSpotlightTerms =
          turSNSiteSpotlightTermRepository.findByTurSNSiteSpotlight(turSNSiteSpotlight);
      turSNSiteSpotlightTermRepository.deleteAllInBatch(turSNSiteSpotlightTerms);

      Set<TurSNSiteSpotlightDocument> turSNSiteSpotlightDocuments =
          turSNSiteSpotlightDocumentRepository.findByTurSNSiteSpotlight(turSNSiteSpotlight);
  ...
  @CacheEvict(
      value = {"spotlight", "spotlight_term"},
      allEntries = true)
  public boolean deleteUnmanagedSpotlight(TurSNJobItem turSNJobItem, TurSNSite turSNSite) {
    if (turSNJobItem.getAttributes().containsKey(TurSNFieldName.ID)) {
      TurSNSiteLocale turSNSiteLocale =
          turSNSiteLocaleRepository.findByTurSNSiteAndLanguage(turSNSite, turSNJobItem.getLocale());
      Set<TurSNSiteSpotlight> turSNSiteSpotlights =
          turSNSiteSpotlightRepository.findByUnmanagedIdAndTurSNSiteAndLanguage(
      ...
      logger.warn(
          "Spotlight ID '{}' of '{}' SN Site ({}) was deleted.",
      ...
      Set<TurSNSiteSpotlight> turSNSiteSpotlights =
          turSNSiteSpotlightRepository.findByProvider(provider);
      turSNSiteSpotlightRepository.deleteAllInBatch(turSNSiteSpotlights);
  ...
  public boolean createUnmanagedSpotlight(TurSNJobItem turSNJobItem, TurSNSite turSNSite) {
    ...
    if (turSNSiteLocale == null) {
      logger.warn(
          "Spotlight ID '{}' of '{}' SN Site was not processed, because {} locale did not found.",
          turSNJobItem.getAttributes().get(TurSNFieldName.ID),
          turSNSite.getName(),
          turSNJobItem.getLocale());
      return false;
      ...
      Set<TurSNSiteSpotlight> turSNSiteSpotlights =
          turSNSiteSpotlightRepository.findByUnmanagedIdAndTurSNSiteAndLanguage(
              id, turSNSite, turSNSiteLocale.getLanguage());
      ...
      try {
        ...
        List<TurSpotlightContent> turSpotlightContents =
            new ObjectMapper().readValue(jsonContent, new TypeReference<>() {});
        ...
        logger.warn(
            "Spotlight ID '{}' of '{}' SN Site ({}) was created.",
            turSNJobItem.getAttributes().get(TurSNFieldName.ID),
            turSNSite.getName(),
            turSNJobItem.getLocale());
  ...
  @NotNull
  private static TurSNSiteSpotlightDocument getTurSNSiteSpotlightDocument(
      TurSpotlightContent turSpotlightContent, TurSNSiteSpotlight turSNSiteSpotlight) {
  ...
  public void addSpotlightToResults(
    ...
    Map<Integer, List<TurSNSiteSpotlightDocument>> turSNSiteSpotlightDocumentMap =
        getSpotlightsFromQuery(context, turSNSite);

    int firstRowPositionFromCurrentPage =
        TurSolrUtils.firstRowPositionFromCurrentPage(context.getTurSEParameters()) + 1;
    int lastRowPositionFromCurrentPage =
        TurSolrUtils.lastRowPositionFromCurrentPage(context.getTurSEParameters()) - 1;

    if (lastRowPositionFromCurrentPage
        > firstRowPositionFromCurrentPage + turSNSiteSearchDocumentsBean.size()) {
    ...
    for (int currentPositionFromList = 0;
        currentPositionFromList < maxPositionFromList;
        currentPositionFromList++) {

      int currentPositionFromCurrentPage =
          currentPositionFromList + firstRowPositionFromCurrentPage;
      ...
      if (turSNSiteSpotlightDocumentMap.containsKey(currentPositionFromCurrentPage)
        ...
        lastRowPositionFromCurrentPage++;
        List<TurSNSiteSpotlightDocument> turSNSiteSpotlightDocuments =
            turSNSiteSpotlightDocumentMap.get(currentPositionFromCurrentPage);
        for (TurSNSiteSpotlightDocument document : turSNSiteSpotlightDocuments) {
          TurSEResult turSEResult =
              turSolr.findById(turSolrInstance, turSNSite, document.getReferenceId());
          if (turSEResult != null) {
            TurSNUtils.addSNDocumentWithPosition(
                context.getUri(),
                fieldExtMap,
                facetMap,
            ...
            TurSNUtils.addSNDocumentWithPosition(
                ...
                facetMap,
                turSNSiteSearchDocumentsBean,
                TurSEResult.builder().fields(fields).build(),
                true,
                currentPositionFromList);
          }
        }
  ...
  public Map<Integer, List<TurSNSiteSpotlightDocument>> getSpotlightsFromQuery(
    ...
    turSpotlightCache
        .findTermsBySNSiteAndLanguage(turSNSite.getName(), context.getLocale())
        .forEach(
            turSNSiteSpotlightTerm -> {
              if (context
                      .getTurSEParameters()
                      .getQuery()
                      .toLowerCase()
    ...
    turSNSiteSpotlights.forEach(
        spotlight -> {
          Set<TurSNSiteSpotlightDocument> turSNSiteSpotlightDocuments =
              turSNSiteSpotlightDocumentRepository.findByTurSNSiteSpotlight(spotlight);
          if (turSNSiteSpotlightDocuments != null && !turSNSiteSpotlightDocuments.isEmpty()) {
            turSNSiteSpotlightDocuments.forEach(
                document -> {
                  if (turSNSiteSpotlightDocumentMap.containsKey(document.getPosition())) {
                    ...
                    turSNSiteSpotlightDocumentMap.put(
                        document.getPosition(), new ArrayList<>(List.of(document)));
                  }
                });
          }
        });
