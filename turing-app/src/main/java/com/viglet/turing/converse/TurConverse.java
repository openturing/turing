...
public class TurConverse {
  ...
  public void saveChatResponseUser(String q, TurConverseChat chat, HttpSession session) {
    boolean hasParameter =
        session.getAttribute(TurConverseConstants.HAS_PARAMETER) != null
    ...
    TurConverseChatResponse chatResponseBot =
        (TurConverseChatResponse) session.getAttribute(TurConverseConstants.PREVIOUS_RESPONSE_BOT);
  ...
  public AbstractMap.SimpleEntry<String, String> getParameterValue(
      String text, TurConverseChat turConverseChat, String intentId) {
    SolrDocumentList termDocumentList =
        turConverseSE.sorlGetParameterValue(text, turConverseChat, intentId);
    if (!termDocumentList.isEmpty()) {
      String parameter =
          (String) termDocumentList.get(0).getFirstValue(TurConverseConstants.PARAMETERS);
  ...
  public void saveChatResponseBot(
      TurConverseChat chat,
    ...
    if (session.getAttribute(TurConverseConstants.PREVIOUS_RESPONSE_USER) != null) {
      TurConverseChatResponse chatResponseUser =
          (TurConverseChatResponse)
              session.getAttribute(TurConverseConstants.PREVIOUS_RESPONSE_USER);
  ...
  public void getChatParameter(
      TurConverseChat chat,
      HttpSession session,
      TurConverseAgentResponse turConverseAgentResponse) {

    int nextParameter =
        session.getAttribute(TurConverseConstants.NEXT_PARAMETER) != null
            ? (int) session.getAttribute(TurConverseConstants.NEXT_PARAMETER)
            : 0;

    String intent =
        session.getAttribute(TurConverseConstants.INTENT) != null
            ? (String) session.getAttribute(TurConverseConstants.INTENT)
            : null;

    SolrDocumentList resultsParameter =
        turConverseSE.solrGetActionAndParameters(chat.getAgent(), intent);
    if (resultsParameter.size() > nextParameter) {
      ...
      if (!resultsParameter.isEmpty()) {
        List<String> prompts =
            (List<String>) firstResultParameter.getFieldValue(TurConverseConstants.PROMPTS);
        if (!prompts.isEmpty()) {
  ...
  public TurConverseAgentResponse interactionNested(
      TurConverseChat chat, String q, String nextContext, HttpSession session) {
    ...
    if (results.isEmpty()) results = turConverseSE.askPhrase(chat.getAgent(), q, nextContext);
    ...
    if (!results.isEmpty()) {
      ...
      if ((boolean) firstResult.getFieldValue(TurConverseConstants.HAS_PARAMETER)) {
        session.setAttribute(
            TurConverseConstants.INTENT, firstResult.getFieldValue(TurConverseConstants.ID));
  ...
  private void getIntentFlow(
    ...
    List<String> contextOutputs =
        (List<String>) firstResult.getFieldValue(TurConverseConstants.CONTEXT_OUTPUT);

    turConverseAgentResponse.setResponse(this.getIntentResponse(chat, firstResult));
    turConverseAgentResponse.setIntentId(
        firstResult.getFieldValue(TurConverseConstants.ID).toString());
    turConverseAgentResponse.setIntentName(
        firstResult.getFieldValue(TurConverseConstants.NAME).toString());
  ...
  private String getIntentResponse(TurConverseChat chat, SolrDocument firstResult) {
    List<String> responses =
        (List<String>) firstResult.getFieldValue(TurConverseConstants.RESPONSES);
    ...
    for (String word : words) {
      if (word.startsWith("$")) {
        ...
        List<TurConverseChatResponse> values =
            turConverseChatResponseRepository.findByChatAndIsUserAndParameterNameOrderByDateDesc(
                chat, true, parameterName);
        if (!values.isEmpty()) {
  ...
  private String getFallbackResponse(TurConverseChat chat) {
    ...
    if (fallbackList.isEmpty()) return TurConverseConstants.FALLBACK_DEFAULT_MESSAGE;
    else {
  ...
  private void getIntentWhenFinishParameters(
      TurConverseChat chat,
      HttpSession session,
      TurConverseAgentResponse turConverseAgentResponse,
      String intent) {
