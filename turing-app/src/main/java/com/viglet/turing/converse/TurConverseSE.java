...
public class TurConverseSE {
  ...
  private Optional<SolrClient> getSolrClient(TurConverseAgent turConverseAgent) {
    ...
    return turSolrInstanceProcess
        .initSolrInstance(turSEInstance, core)
        .map(TurSolrInstance::getSolrClient);
  }
  ...
  public SolrDocumentList askPhrase(
      TurConverseAgent turConverseAgent, String q, String nextContext) {
    return turSolrAskPhrase(turConverseAgent, q, nextContext, TurConverseConstants.INTENT_TYPE);
  }

  public SolrDocumentList askPhraseFallback(
      TurConverseAgent turConverseAgent, String q, String nextContext) {
    return turSolrAskPhrase(
        turConverseAgent, q, nextContext, TurConverseConstants.FALLBACK_INTENT_TYPE);
  }

  private SolrDocumentList turSolrAskPhrase(
      TurConverseAgent turConverseAgent, String q, String nextContext, String type) {
    ...
    if (nextContext != null) {
      ...
      query.addFilterQuery(
          keyValueQuery("-".concat(TurConverseConstants.CONTEXT_INTPUT), SOLR_ANY, true));
    }
  ...
  private SolrDocumentList executeSolrQuery(
      TurConverseAgent turConverseAgent, SolrQuery queryParameter) {
    try {
  ...
  SolrDocumentList solrGetActionAndParameters(TurConverseAgent turConverseAgent, String intent) {
    ...
    queryParameter.addFilterQuery(
        keyValueQuery(TurConverseConstants.TYPE, TurConverseConstants.PARAMETER_TYPE, true));
    queryParameter.addFilterQuery(
        keyValueQuery(TurConverseConstants.AGENT, turConverseAgent.getId(), true));
  ...
  SolrDocumentList solrGetIntent(TurConverseAgent turConverseAgent, String intent) {
    SolrQuery query = new SolrQuery();
    query.addFilterQuery(
        keyValueQuery(TurConverseConstants.TYPE, TurConverseConstants.INTENT_TYPE, true));
    query.addFilterQuery(keyValueQuery(TurConverseConstants.AGENT, turConverseAgent.getId(), true));
  ...
  public SolrDocumentList sorlGetParameterValue(
      String text, TurConverseChat turConverseChat, String intentId) {
    ...
    query.addFilterQuery(
        keyValueQuery(TurConverseConstants.TYPE, TurConverseConstants.TERM_TYPE, true));
    query.addFilterQuery(
        keyValueQuery(TurConverseConstants.AGENT, turConverseChat.getAgent().getId(), true));
  ...
  public void index(TurConverseIntent turConverseIntent) {
    ...
    turSolrInstanceProcess
        .initSolrInstance(
            turConverseIntent.getAgent().getTurSEInstance(), turConverseIntent.getAgent().getCore())
        .ifPresent(
            solrInstance -> {
  ...
  private void indexIntent(TurConverseIntent turConverseIntent, SolrClient solrClient) {
    ...
    document.addField(TurConverseConstants.AGENT, turConverseIntent.getAgent().getId());
    document.addField(
        TurConverseConstants.TYPE,
        turConverseIntent.isFallback()
            ? TurConverseConstants.FALLBACK_INTENT_TYPE
    ...
    Set<TurConverseEntity> entities =
        turConverseEntityRepository.findByAgent(turConverseIntent.getAgent());
    ...
    document.addField(
        TurConverseConstants.HAS_PARAMETERS, !turConverseIntent.getParameters().isEmpty());
    ...
    if (phrase.getText().contains(entityName(entity.getName()))) {
      for (TurConverseEntityTerm term : entity.getTerms()) {
        Set<TurConverseParameter> parameters =
            turConverseParameterRepository.findByIntentAndEntity(
                turConverseIntent, entityName(entity.getName()));
        phrasesVariationfromSynonyms(
            turConverseIntent, solrClient, document, phrase, entity, term, parameters);
      }
    }
  }

  private void phrasesVariationfromSynonyms(
      TurConverseIntent turConverseIntent,
      SolrClient solrClient,
      SolrInputDocument document,
      TurConversePhrase phrase,
      TurConverseEntity entity,
      TurConverseEntityTerm term,
      Set<TurConverseParameter> parameters) {
    for (String synonym : term.getSynonyms()) {
      ...
      if (!parameters.isEmpty()) {
        for (TurConverseParameter parameter : parameters) {
          indexTerm(
              turConverseIntent, solrClient, entity, term, synonym, phraseFormatted, parameter);
        }
      }
    }
  }

  private void indexTerm(
      TurConverseIntent turConverseIntent,
      SolrClient solrClient,
      TurConverseEntity entity,
      TurConverseEntityTerm term,
      String synonym,
      String phraseFormatted,
      TurConverseParameter parameter) {
    if (logger.isDebugEnabled()) logger.debug("Have parameters to Entity: {}", entity.getName());
    SolrInputDocument termDocument = new SolrInputDocument();
    termDocument.addField(
        TurConverseConstants.ID,
    ...
    termDocument.addField(
        TurConverseConstants.PARAMETERS, keyValueQuery(parameter.getName(), term.getName(), false));
            ...
            solrInstance -> {
              try {
                solrInstance
                    .getSolrClient()
                    .deleteByQuery(
                        keyValueQuery(TurConverseConstants.ID, turConverseIntent.getId(), true));
