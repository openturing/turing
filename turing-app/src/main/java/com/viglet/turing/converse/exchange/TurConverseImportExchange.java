...
public class TurConverseImportExchange {
  ...
  @Inject
  public TurConverseImportExchange(
  ...
  public TurConverseAgentExchange importFromMultipartFile(@Nonnull MultipartFile multipartFile) {
    ...
    if (extractFolder != null) {
      ...
      if (!(new File(extractFolder, AGENT_FILE).exists())
      ...
      TurConverseAgentExchange turConverseAgentExchange =
          extractAgentExchange(extractFolder, mapper);

      TurConverseAgent turConverseAgent =
          saveConverseAgent(multipartFile, turConverseAgentExchange);
      ...
      final File folder =
          new File(extractFolder.getAbsolutePath().concat(File.separator + "intents"));
  ...
  private void extractEntityEntriesExchange(File extractFolder, ObjectMapper mapper) {
    ...
    try {
      turConverseEntityEntriesExchange =
          mapper.readValue(
              new FileInputStream(
                  extractFolder
                      .getAbsolutePath()
  ...
  private TurConverseAgent saveConverseAgent(
      MultipartFile multipartFile, TurConverseAgentExchange turConverseAgentExchange) {
  ...
  private TurConverseAgentExchange extractAgentExchange(File extractFolder, ObjectMapper mapper) {
    ...
    try {
      turConverseAgentExchange =
          mapper.readValue(
              new FileInputStream(
                  extractFolder.getAbsolutePath().concat(File.separator + AGENT_FILE)),
  ...
  private void readFileFromFolder(
      TurConverseAgent turConverseAgent, final ObjectMapper mapper, final File fileEntry) {
    ...
    if (fileEntry.getName().contains("_usersays_")) {
      ...
      TurConverseIntentExchange turConverseIntentExchange =
          extractIntentExchange(mapper, fileEntry);
  ...
  private TurConverseIntentExchange extractIntentExchange(
      final ObjectMapper mapper, final File fileEntry) {
    TurConverseIntentExchange turConverseIntentExchange = new TurConverseIntentExchange();
    try {
      turConverseIntentExchange =
          mapper.readValue(new FileInputStream(fileEntry), TurConverseIntentExchange.class);
  ...
  private void extractIntentPhrasesExchange(final ObjectMapper mapper, final File fileEntry) {
    ...
    try {
      turConverseIntentPhrasesExchange =
          mapper.readValue(new FileInputStream(fileEntry), TurConverseIntentPhrasesExchange.class);
  ...
  private void savePhrases(
      final ObjectMapper mapper, final File fileEntry, TurConverseIntent turConverseIntent) {
    File phrasesFile =
        new File(fileEntry.getAbsolutePath().replace(".json", "_usersays_pt-br.json"));
    if (phrasesFile.exists()) {
      TurConverseIntentPhrasesExchange turConverseIntentPhrasesExchange;
      try {
        turConverseIntentPhrasesExchange =
            mapper.readValue(
                new FileInputStream(phrasesFile), TurConverseIntentPhrasesExchange.class);
  ...
  private void saveResponses(
      TurConverseIntentExchange turConverseIntentExchange, TurConverseIntent turConverseIntent) {
    for (TurConverseIntentResponseExchange response : turConverseIntentExchange.getResponses()) {
      for (TurConverseIntentMessageExchange message : response.getMessages()) {
        if (!message.getSpeech().isEmpty()) {
          TurConverseResponse turConverseResponse =
              new TurConverseResponse(message.getSpeech().get(0));
  ...
  private TurConverseIntent saveIntent(
      TurConverseAgent turConverseAgent, TurConverseIntentExchange turConverseIntentExchange) {
  ...
  public TurConverseAgentExchange importFromFile(File file) {
    ...
    try (FileInputStream input = new FileInputStream(file)) {
      MultipartFile multipartFile =
          new MockMultipartFile(file.getName(), IOUtils.toByteArray(input));
